...
@SpringBootTest
class KaKaoBlogRequesterTest {
    @Autowired
    private KaKaoBlogRequester requester;

    @Test
    public void search() {
        BlogSearchRequest request = new BlogSearchRequest(BlogType.KAKAO, "테스트", Sort.ACCURACY, 10, 1);
    ...
    public void search_paging() {
        int size = 5;
        int page = 2;
        BlogSearchRequest request1 = new BlogSearchRequest(BlogType.KAKAO, "하늘", Sort.ACCURACY, size, page);
        BlogSearchResponse response1 = requester.search(request1);

        BlogSearchRequest request2 = new BlogSearchRequest(BlogType.KAKAO, "하늘", Sort.ACCURACY, size * page, 1);
    ...
    public void search_invalid_params() {
        assertThrows(BlogSearchClientException.class, () -> {
            BlogSearchRequest request = new BlogSearchRequest(BlogType.KAKAO, "___", Sort.ACCURACY, 10, 1);
            requester.search(request);
        });

        assertThrows(BlogSearchClientException.class, () -> {
            BlogSearchRequest request = new BlogSearchRequest(BlogType.KAKAO, "___", Sort.ACCURACY, 10, Integer.MAX_VALUE);
            requester.search(request);
        });
